...
const AddItemModal = ({ isOpen, onClose, onAddItem, onNavigate }) => {
  ...
  const needsExpiryDate = formData.category === 'fridge' || formData.category === 'freezer';
  ...
  const getContainerClasses = (fieldName) => {
    ...
    const isFocused = focusedField === fieldName || (fieldName === 'category' && isCategoryOpen);
  ...
  const baseInputClasses = "w-full py-3 pr-4 bg-transparent font-medium text-[var(--color-black)] !outline-none focus:!outline-none !border-none focus:!border-none !ring-0 focus:!ring-0 !shadow-none focus:!shadow-none";
              ...
              <div className="flex p-1.5 mb-6 bg-gray-100 rounded-xl relative gap-1.5">
                <button
                  ...
                  className={`flex-1 flex flex-col items-center justify-center py-2 text-xs font-bold rounded-lg transition-all z-10 ${
                    mode === 'manual' ? 'text-[var(--color-black)]' : 'text-gray-500 hover:text-gray-700'
                  }`}
                >
                ...
                <button
                  ...
                  className={`flex-1 flex flex-col items-center justify-center py-2 text-xs font-bold rounded-lg transition-all z-10 ${
                    mode === 'camera' ? 'text-[var(--color-black)]' : 'text-gray-500 hover:text-gray-700'
                  }`}
                >
                ...
                <button
                  ...
                  className={`flex-1 flex flex-col items-center justify-center py-2 text-xs font-bold rounded-lg transition-all z-10 ${
                    mode === 'scan' ? 'text-[var(--color-black)]' : 'text-gray-500 hover:text-gray-700'
                  }`}
                >
                  ...
                  {mode === 'manual' && (
                    ...
                    >
                      ...
                      <div className="relative">
                        <label htmlFor="name" className={commonLabelClasses('name')}>Item Name</label>
                        <div className={getContainerClasses('name')}>
                          <div className="flex items-center pl-4 pr-3 py-3 text-gray-400">
                             <Tag size={18} className={focusedField === 'name' ? 'text-blue-500' : ''} />
                             <span className="ml-3 text-gray-300">|</span>
                          </div>
                          <input
                            id="name" type="text" name="name" placeholder="e.g., Organic Milk"
                      ...
                      <div className="grid grid-cols-2 gap-4">
                        {/* Quantity Input */}
                        <div className="relative">
                          <label htmlFor="quantity" className={commonLabelClasses('quantity')}>Quantity</label>
                          <div className={getContainerClasses('quantity')}>
                            <div className="flex items-center pl-4 pr-3 py-3 text-gray-400">
                               <Hash size={18} className={focusedField === 'quantity' ? 'text-blue-500' : ''} />
                               <span className="ml-2 text-gray-300">|</span>
                            </div>
                            <input
                              id="quantity" type="number" name="quantity" min="1" placeholder="1"
                        ...
                        <div className="relative">
                          <label className={commonLabelClasses('category')}>Category</label>
                          <div 
                          ...
                          >
                            <div className="w-full flex items-center justify-between py-3 pl-4 pr-3 font-medium text-[var(--color-black)] select-none">
                              ...
                              <ChevronDown size={18} className={`text-gray-400 transition-transform duration-300 ${isCategoryOpen ? 'rotate-180 text-blue-500' : ''}`} />
                            </div>
                          </div>
                                ...
                                {categories.map((cat) => (
                                  <div
                                    ...
                                    className={`px-4 py-3 cursor-pointer text-sm font-bold transition-colors ${
                                      formData.category === cat.value ? 'bg-blue-50 text-blue-600' : 'text-gray-700 hover:bg-gray-50'
                                    }`}
                                  >
                                    {cat.label}
                                  </div>
                      ...
                      {/* Expiry Input */}
                      <div className="relative">
                        <label htmlFor="expiry" className={commonLabelClasses('expiry')}>
                          Expiry Date {needsExpiryDate ? '(Required)' : '(Optional)'}
                        </label>
                        <div className={getContainerClasses('expiry')}>
                          <div className="flex items-center pl-4 pr-3 py-3 text-gray-400">
                             <Calendar size={18} className={focusedField === 'expiry' ? 'text-blue-500' : ''} />
                             <span className="ml-3 text-gray-300">|</span>
                          </div>
                          <input
                            id="expiry" type="date" name="expiry"
                            value={formData.expiry} onChange={handleInputChange}
                            onFocus={() => setFocusedField('expiry')}
                            onBlur={() => validateOnBlur('expiry', formData.expiry)}
                            className={baseInputClasses}
                          />
                        </div>
                        {errors.expiry && (
                    ...
                    >
                      ...
                      {capturedImage ? (
                        <div className="flex gap-2 w-full">
                           <button type="button" onClick={() => setCapturedImage(null)} className="flex-1 py-3 bg-gray-100 text-gray-700 rounded-xl font-bold hover:bg-gray-200 transition">
                             Retake
                           </button>
                        </div>
                      ) : (
                      ...
                      <p className="text-xs text-gray-500 font-medium">Point camera at your grocery items</p>
                    </motion.div>
                  )}
                    ...
                    >
                      <label className="flex flex-col items-center justify-center w-full h-48 border-2 border-dashed border-[var(--color-blue)] bg-[var(--color-blue)]/5 hover:bg-[var(--color-blue)]/10 rounded-2xl cursor-pointer transition-colors group">
                        <div className="flex flex-col items-center justify-center pt-5 pb-6">
                          ...
                          <p className="mb-2 text-sm font-bold text-[var(--color-black)]">Select an image from gallery</p>
                      ...
                      {receiptFile && (
                        <div className="p-3 bg-green-50 text-green-700 rounded-lg text-sm font-bold flex items-center gap-2 border border-green-200">
                          <CameraIcon size={16} /> Image attached: {receiptFile.name}
                        </div>
                      )}
                    </motion.div>
                  )}
